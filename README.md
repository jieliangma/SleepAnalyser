...
### 房间管理
- **多房间支持** — 添加、删除、重命名房间，每个房间独立校准数据
- **房间切换** — 选择当前房间，睡眠分析自动加载对应校准数据
- **独立校准** — 每个房间单独校准噪声基底和麦克风增益

### 噪声管理
- **实时采集模式** — 实时录制环境音，C 引擎自动分类并打标签，录制中显示实时波形动画
- **多层分解** — 同一时刻可存在多种重叠噪声（如交通+空调+风声），自动拆分独立标记
- **人工标注** — 在波形图上拖拽选择时间范围，手动指定噪声类型
- **人工矫正** — 编辑噪声类型、添加备注、确认标注
- **一键确认** — 批量确认所有噪声段并自动喂给 ML 训练
- **噪声类型管理** — 自定义噪声类型：名称、颜色、图标、关联声音片段
- **声音关联** — 每种噪声类型可关联多个声音样本，在类型管理页可播放
- **ML 自动训练** — 确认的标注积累 ≥10 条后自动重训练噪声分类模型
- **分类筛选** — 按噪声类型过滤查看
...
麦克风 (AVAudioEngine, 16kHz mono)
  → 预处理 (DC 去除 / 预加重 / AGC)
    → 噪声抑制 (房间校准噪声基底 + 自适应频谱减法 + 麦克风增益校正)
      → C 噪声分离 (前景呼吸信号 / 背景环境噪声)
        ├→ 前景信号 → 带通滤波(200-2000Hz) → 呼吸频率估计
        ├→ 前景信号 → 特征提取 (MFCC / 频谱质心 / ZCR / RMS)
        ├→ 背景噪声 → 多层分解 → 噪声类型上下文标记
        └→ 事件检测 (打鼾 / 干扰 / 语音 / 离床)
          → Core ML 推理 (特征 + 噪声上下文 + 房间校准状态)
            → HMM 时序平滑 → 90分钟周期约束
